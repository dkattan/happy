...
foreach ($target in $targets) {
    ...
    if ($outputLog) {
        $logsToTail += @{ Name = "$($target.Name) output"; Path = $outputLog.FullName }
    }
}

if (-not $logsToTail.Count) {
    Write-Error 'No Happy VS Code Bridge output logs found under %APPDATA%\\Code\\logs or %APPDATA%\\Code - Insiders\\logs.'
    exit 1
}

Write-Host 'Tailing Happy VS Code Bridge output logs (Ctrl+C to stop):'
